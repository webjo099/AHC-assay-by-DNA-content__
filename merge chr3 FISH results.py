...
# define a function to search for chr3 FISH results files and merge them
def findAndMerge(filelist):
    # Prepare a dataframe
    df_out = pd.DataFrame(columns=['directory', 'image', 'nul', 'single', 'multiple','genotype'])
    for file in filelist:
        ...
        if "FISH results.txt" in file_name:
            ...
            text_file.close()
            nul = int(re.findall('\d+',lines[-3])[0])
            single = int(re.findall('\d+',lines[-2])[0])
            multiple = int(re.findall('\d+',lines[-1])[0])
            imagename = file.replace('\\', '/').replace(dirName, '').split(' FISH results.txt')[0]
            df_temp = pd.DataFrame([[dirName, imagename, nul, single, multiple]], columns=['directory', 'image', 'nul', 'single', 'multiple'])
            df_out = df_out.append(df_temp, sort=False, ignore_index=True)
            print("It contains " + str(nul) + " nuls, " + str(single) + " singles and " + str(multiple) + " multiple dote containing nuclei :o")
    return df_out
...
# Prepare a dataframe
df_merged = pd.DataFrame(columns=['directory', 'image', 'nul', 'single', 'multiple','genotype'])
...
df_merged['aneuploidy ratio'] = ((2*df_merged['multiple'])/(df_merged['single']+2*df_merged['multiple']))

#ask the user where to save the final dataframe as .xlsx file
save_path = filedialog.asksaveasfilename(title='Save compiled results as ...',defaultextension = '.xlsx',initialdir = dirName, initialfile = "compiled chr3 FISH results");
